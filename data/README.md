...
| `streams` | **Да** | Содержит массив урезанных объектов "Стрим". Должен содержать ключ `twitch`, присутствующий в `streams.json`. Также нужно заполнить поле `name` (см. чуть ниже). Дополнительно можно переопределить параметры `start`, `end`, `offset` и `note` объекта "Стрим", если это требуется в контексте прохождения. |
| `streams[*]`.`name` | **Да** | Заголовок стрима на странице прохождения. Для прохождений типа `list` и стримов с несколькими играми названия игр можно разделять слэшем **с пробелами** (' / '). |
