...
| `timecodes` | Нет | Объект, содержащий пары ключ-значение. Ключ -- время в формате HH:MM:SS. Значение -- название таймкода. (см. пример выше) |
| `start` | Нет | Таймкод, с которого должно начинаться воспроизведение стрима. Эта опция полезна в случаях, когда на записи перед основной игрой идёт другая. |
| `soft_start` | Нет | Более мягкая версия `start`, которая только покажет таймкод, но перематывать видео не будет. Эта опция полезна для тех, кто хочет пропустить вступление стрима, но не побеспокоит тех, кто его смотрит. |
