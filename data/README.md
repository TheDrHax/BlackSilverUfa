...
```
"654380816": { "youtube": "ZBHv3E0ZDHo" },
"655265437": { "youtube": "ZBHv3E0ZDHo", "offset": "-7:32:13" },
"655528246": { "youtube": "ZBHv3E0ZDHo", "offset": "-14:13:20" },
"656257835": { "youtube": "ZBHv3E0ZDHo", "offset": "-17:13:32" },
"654380816,655265437,655528246,656257835": { "note": "4 стрима были склеены в одну запись." },
```

Если в `streams.json` вместо ключа стрима указать ключи уже существующих стримов, разделённые запятой, то будет создан ещё один стрим, виртуально объединяющий их в один таймлайн. Субтитры и таймкоды также будут склеены соответствующим образом, принимая во внимание вырезанные моменты и смещения сегментов.

Объединённые стримы игнорируют параметры `youtube`, `direct`, `cuts`, `official` и `offset`. Значения этих параметров берутся из первого сегмента первого указанного стрима.

Объединённые стримы поддерживают только один сегмент. При указании нескольких сегментов будет возвращена ошибка.

Все перечисленные стримы должны использовать один и тот же источник записи, но с разными смещениями.
