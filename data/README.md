...
| `streams` | **Да** | Содержит массив урезанных объектов "Стрим". Должен содержать ключ `twitch`. Дополнительно можно переопределить параметры `start`, `end`, `offset` и `note` объекта "Стрим", если это требуется в контексте прохождения. |
