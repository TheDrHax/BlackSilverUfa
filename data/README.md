...
{ "name": "1 (части 2-6)", "twitch": ["728162478", "728192109", "728196605", "728204189", "728213819"] },
```

| Поле | Обязательное | Описание |
| ---- | :----------: | -------- |
| `name` | **Да**, конфликтует с `subrefs` | Заголовок стрима на странице прохождения. |
| `twitch` | **Да** | ID стрима, на который ссылается данный сегмент. Данный ID также должен присутствовать в файле `streams.json`. Также возможно указание списка ID, если соответствующие сегменты используют одну и ту же запись - в таком случае субтитры и таймкоды будут склеены. |
