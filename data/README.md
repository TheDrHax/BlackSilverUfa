...
| `vk` | Нет | Один или несколько ID видео в VK. При использовании этого поля VK считается первоначальным источником стрима (заменяя Twitch). Этот параметр используется для добавления стримов, у которых утеряны ID VOD'а и логи чата. |
